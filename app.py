...
def main():
    ...
    - 결재는 발신부서와 수신부서의 개념을 뜻하지만, 비즈박스 특성상 이를 구현할 수 없으므로 결재자와 합의자로 구분합니다.
      > 수신부서의 팀원이 결재 라인에 들어가는 이유는 수신부서에서도 **실무적 처리**를 해야하기 때문이며, 결재 라인이 길어지는 것과 무관합니다. 
    ...
    if selected_document in documents_requiring_amount:
        ...
        # **주의 문구 추가**: '구매(용역)요청서-계좌이체/법인카드', '지출승인요청서(법인카드)', '회의비 사전 신청서' 선택 시
        if selected_document in ['구매(용역)요청서-계좌이체/법인카드', '지출승인요청서(법인카드)', '회의비 사전 신청서']:
            st.warning('국책(연구)카드라면 전략실(해그리드) 합의를 GWP센터 합의 전에 추가해주세요.')
