...
def main():
    ...
    st.markdown("""
        ### 결재라인에 대한 설명
             - 결재라인은 문서의 결재 과정을 나타내는 흐름도이며, 각 문서의 결재 과정에 따라 결재자와 합의자가 결정됩니다.
             - 결재는 발신부서와 수신부서의 개념을 뜻하지만 비즈박스 특성상 구현을 할 수 없으므로, 우리는 결재자와 합의자로 구분합니다.
             - 기본적인 흐름은 **발신부서 결재완료** 후, **수신부서 합의완료**의 흐름입니다.
             - 다만 최상위 결재자, **아이언맨**과 **백호**의 경우 **수신부서** -보통(GWP센터)-의 합의가 끝난 후 결재를 진행합니다.
    """)
...
def generate_approval_line(selected_center, selected_team, selected_role, selected_document, amount):
    ...
    # 만약 center_head와 '중앙기술연구소장'이 동일인물이라면, center_head를 '중앙기술연구소장'으로 변경
